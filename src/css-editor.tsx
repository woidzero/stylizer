...
class CSSEditor extends React.Component<ICSSEditorProps, ICSSEditorStates> {
  ...
  constructor(props: ICSSEditorProps) {
    ...
    this.settings.addButton(
      "button-1",
      "Open CSS Editor to see your current CSS",
      "Open CSS Editor",
      () => this.open()
    );
    this.settings.addInput(
      "font-size",
      "Font size for the CSS Editor (in pixels)",
      "12px",
      () => {
        this.setState({
    ...
    this.settings.addInput(
      "open-hotkey",
      "Quickly open CSS Editor with an hotkey",
      "F12",
      () => {
  ...
  open() {
    this.setState((state) => {
      return {
        ...
        width: Math.max(
          this.settings.getFieldValue<number>("editor-width"),
          100
        ),
        height: Math.max(
          this.settings.getFieldValue<number>("editor-height"),
          100
        ),
      };
    });
  ...
  setPosition(pos?: { x: number; y: number }) {
    this.setState((state) => {
      if (!pos) pos = { x: state.x, y: state.y };
  ...
  setSize(size?: { width: number; height: number }) {
    this.setState((state) => {
      if (!size) size = { width: state.width, height: state.height };
    ...
    return (
      <>
        ...
        <div
          ...
          style={{ display: this.state.visible ? undefined : "none" }}
        >
          <Rnd
          ...
          >
            <div
              ...
              className={styles.draggableChild}
            >
              <div
                className={
                  "main-playlistEditDetailsModal-header " +
              ...
              <div
                ...
                onMouseOver={() => {
                  this.setState({ mouseOverEditor: true });
                }}
                onMouseLeave={() => {
                  this.setState({ mouseOverEditor: false });
                }}
              >
                <AceEditor
                  mode="css"
